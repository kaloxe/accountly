...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->setXY(60, 15);
		$this->Cell(100, 8, 'Reporte de totales', 0, 1, 'C', 0);
...
switch ($_SESSION["cuenta"]) {
	...
	case "1":
		...
		$sql1 = "SELECT count(id_font) as count FROM font WHERE  id_user=$id_user";
		$sql2 = "SELECT SUM(amount) as total FROM `font` WHERE id_user=$id_user";
		break;
	case "2":
		$sql = "SELECT description as name_font, amount FROM `debt` WHERE id_user=$id_user";
		$sql1 = "SELECT count(id_debt) as count FROM debt WHERE  id_user=$id_user";
		$sql2 = "SELECT SUM(amount) as total FROM `debt` WHERE id_user=$id_user";
		break;
	default:
		$sql = "SELECT * FROM `font` WHERE id_user=$id_user UNION SELECT * FROM `debt` WHERE id_user=$id_user";
		$sql1 = "SELECT count(t.cantidad) as count FROM (SELECT id_font as cantidad FROM `font` WHERE id_user=$id_user UNION SELECT id_debt as cantidad FROM `debt` WHERE id_user=$id_user) t";
...
$data2 = new Conexion();
$conexion2 = $data2->conect();

$result2 = $conexion2->prepare($sql2);
$result2->execute();
...
$pdf->Cell(50, 8, 'Descripcion', 1, 0, 'C', 0);
$pdf->Cell(50, 8, 'Monto', 1, 1, 'C', 0);
...
for ($i = 0; $i < count($data); $i++) {
	...
	$pdf->Row(array($data[$i]['name_font'], $data[$i]['amount']), 55);
}
$pdf->SetX(55);
$pdf->Cell(50, 8, 'Monto', 1, 0, 'C', 0);
$pdf->Cell(50, 8, $data2[0]['total'], 1, 1, 'C', 0);
